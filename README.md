...
- A released `docmethis-check` installation for local tutorials
...
. .venv/bin/activate
python -m pip install "docmethis-check==0.1.0"
```

The exact Check release is part of the tutorial reproducibility contract. Use
the version named here rather than an unpinned latest release.

Run the small application tests with the standard library:
...
The published tutorial series starts from the frozen `tutorials-v0.1.0` base:
...
For a tutorial still being developed, replace `tutorials-v0.1.0` with
`origin/main`. The `setup/*` branches are available as remote-tracking branches
...
All commands in the guides use setup branch names or the global series tag,
never commit SHAs. `main` is the moving base for tutorials in development;
`tutorials-v0.1.0` is the reproducible base for the published video series.
There are no scenario tags or tutorial branches.
...
For a tutorial under development, use the current remote baseline instead:
...
Open the matching file under `docs/tutorials/`. The published guides reset to
`tutorials-v0.1.0`; development copies can reset to `origin/main`. Each guide
...
There is no separate survey mode in this playground. The full catchup tutorial
creates temporary documentation debt, inspects the whole repository with the
empty-tree range, and optionally cleans it with Fix. Its core command is:
...
See [`docs/tutorials/full-catchup.md`](docs/tutorials/full-catchup.md) for the
complete workflow and the reset step that abandons its temporary commits.
